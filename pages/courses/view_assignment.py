...
@app.route('/view_assignment<args>', methods=['GET', 'POST'])
def view_assignment(args):
    args = eval(args)
    course_id = args['course_id']
    assignment_id = args['assignment_id']

    if not is_allowed_course_access(course_id=course_id): return redirect(url_for('home'))
    assignment_page = database_conn.get_Course_Homework_by_ID(homework_id=assignment_id)
    
    return render_template(
        'view_pdf.html',
        menu_data = MENU_LINKS,
        site_data = SITE_DATA,
        link = assignment_page['pdf_link']
    )
